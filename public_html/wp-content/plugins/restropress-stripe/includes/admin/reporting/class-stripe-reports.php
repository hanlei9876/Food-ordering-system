...
class RPRESS_Stripe_Reports {
	...
	public function __construct() {
		...
		//Graph Reports
		add_filter( 'rpress_report_views', array( $this, 'add_stripe_card_reports_view' ) );
		add_action( 'rpress_reports_view_stripe', array( $this, 'display_stripe_report' ) );
	...
	public function add_stripe_card_reports_view( $views ) {
		$views['stripe'] = __( 'Stripe Reports', 'rpstripe' );
	...
	public function get_stripe_payments_by_date( $day = null, $month = null, $year = null, $hour = null, $include_taxes = false ) {
		...
		$args = apply_filters( 'rpress_get_stripe_payments_by_date', array(
			'nopaging'    => true,
		...
		if ( $payments ) {
			foreach ( $payments as $payment ) {
				...
				$existing = $payment->get_meta( '_rpress_stripe_used_existing_card' );
				$tax      = 0;
	...
	public function display_stripe_report() {
		...
		if ( ! current_user_can( 'view_shop_reports' ) ) {
			wp_die( __( 'You do not have permission to view this data', 'rpstripe' ), __( 'Error', 'rpstripe' ), array( 'response' => 401 ) );
		}
		...
		$data = array(
			__( 'New Card Earnings', 'rpstripe' )                => $new_cards_earnings,
			__( 'Existing Card Earnings', 'rpress-recurring' )  => $existing_cards_earnings,
			__( 'New Card Purchases', 'rpress-recurring' )      => $new_cards_counts,
			__( 'Existing Card Purchases', 'rpress-recurring' ) => $existing_cards_counts,
		);
		...
		<?php do_action( 'rpress_subscription_reports_graph_before' ); ?>
		<div class="metabox-holder">
			<div class="postbox">
				<h3><span><?php _e( 'Stripe Card Types Report', 'rpstripe' ); ?></span></h3>
				...
				<div class="inside">
					...
					<p class="rpress_graph_totals">
						<strong>
							<?php
							_e( 'Gross earnings for period shown: ', 'rpstripe' );
							echo rpress_currency_filter( rpress_format_amount( $new_earnings  + $existing_earnings) );
							?>
						</strong>
					</p>
					...
					<p class="rpress_graph_totals">
						<strong>
							<?php
							_e( 'New card earnings: ', 'rpstripe' );
							echo rpress_currency_filter( rpress_format_amount( $new_earnings ) );
							?>
						</strong>
					</p>
					...
					<p class="rpress_graph_totals">
						<strong>
							<?php
							_e( 'Existing card earnings: ', 'rpstripe' );
							echo rpress_currency_filter( rpress_format_amount( $existing_earnings ) );
							?>
						</strong>
					</p>

					<p class="rpress_graph_totals">
						<strong><?php _e( 'Total sales for period shown: ', 'rpstripe' ); echo rpress_format_amount( $new_cards + $existing_cards, false ); ?></strong>
					</p>

					<p class="rpress_graph_totals">
						<strong>
							<?php
							_e( 'Total payments with new cards: ', 'rpstripe' );
							echo rpress_format_amount( $new_cards, false );
							?>
						</strong>
					</p>
					...
					<p class="rpress_graph_totals">
						<strong>
							<?php
							_e( 'Total payments with existing cards: ', 'rpstripe' );
							echo rpress_format_amount( $existing_cards, false );
							?>
						</strong>
					</p>

					<?php do_action( 'rpress_subscription_reports_graph_additional_stats' ); ?>
		...
		<?php do_action( 'rpress_subscription_reports_graph_after' ); ?>

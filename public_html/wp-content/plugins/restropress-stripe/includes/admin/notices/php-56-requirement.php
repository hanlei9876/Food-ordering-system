...
<p>
	<strong><?php esc_html_e( 'RestroPress Stripe Payment Gateway is increasing its PHP version requirement.', 'rpstripe' ); ?></strong>
</p>
...
<p>
	...
	echo wp_kses(
		sprintf(
			...
			__( 'RestroPress Stripe Payment Gateway will be increasing its PHP requirement to version %1$s or higher in an upcoming release. It looks like you\'re using version %2$s, which means you will need to %3$supgrade your version of PHP to allow the plugin to continue to function%4$s. Newer versions of PHP are both faster and more secure. The version you\'re using %5$sno longer receives security updates%6$s, which is another great reason to update.', 'rpstripe' ),
...
<p>
	<button id="rpress-stripe-php-56-read-more" class="button button-secondary button-small"><?php esc_html_e( 'Read More', 'rpstripe' ); ?></button>

	<script>
	document.getElementById( 'rpress-stripe-php-56-read-more' ).addEventListener( 'click', function( e ) {
		e.preventDefault();
...
<div style="display: none;">
	...
	<p>
		<strong><?php esc_html_e( 'Which version should I upgrade to?', 'rpstripe' ); ?></strong>
	</p>
	...
	<p>
		<strong><?php esc_html_e( 'Need help upgrading? Ask your web host!', 'rpstripe' ); ?></strong>
	</p>
	...
	<?php
		echo wp_kses(
			...
			sprintf(
				__( 'Many web hosts can give you instructions on how/where to upgrade your version of PHP through their control panel, or may even be able to do it for you. %1$sRead more about updating PHP%2$s.', 'rpstripe' ),

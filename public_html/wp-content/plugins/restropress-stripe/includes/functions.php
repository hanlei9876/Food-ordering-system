 ...
 * @since 1.1
 *
 * @throws \RPRESS_Stripe_Utils_Exceptions_Stripe_Object_Not_Found When attempting to call an object or method that is not available.
 ...
 */
function rpress_stripe_api_request( $object, $method, $args = null ) {
	$api = new RPRESS_Stripe_API();
 ...
 * @return bool
 */
function rpress_stripe_existing_cards_enabled() {
	$use_existing_cards = rpress_get_option( 'stripe_use_existing_cards', false );
 ...
 * @return array
 */
function rpress_stripe_get_existing_cards( $user_id = 0 ) {
	if ( empty( $user_id ) ) {
		return array();
	}
	...
	$stripe_customer_id = rpress_stripe_get_stripe_customer_id( $user_id );

	if ( ! empty( $stripe_customer_id ) ) {
		try {
			$stripe_customer = rpress_stripe_api_request( 'Customer', 'retrieve', $stripe_customer_id );

			if ( isset( $stripe_customer->deleted ) && $stripe_customer->deleted ) {
				return $customer_cards;
			}

			$payment_methods = rpress_stripe_api_request( 'PaymentMethod', 'all', array(
				'type'     => 'card',
				'customer' => $stripe_customer->id,
				'limit'    => 100,
			) );

			$cards = rpress_stripe_api_request( 'Customer', 'allSources', $stripe_customer->id, array(
				'limit' => 100,
			)	);
 ...
 *
 * @since  1.1
 * @since  1.1              Added lazy load for moving to customer meta and ability to look up by customer ID.
 ...
 */
function rpress_stripe_get_stripe_customer_id( $id_or_email, $by_user_id = true ) {
	$stripe_customer_id = '';
	$meta_key           = rpress_stripe_get_customer_key();

	if ( is_email( $id_or_email ) ) {
		$by_user_id = false;
	}

	$customer = new RPRESS_Customer( $id_or_email, $by_user_id );
	if ( $customer->id > 0 ) {
	...
	if ( empty( $stripe_customer_id ) ) {
		...
		if ( $user ) {
			...
			$customer = new RPRESS_Customer( $user->user_email );
	...
	if ( empty( $stripe_customer_id ) && class_exists( 'RPRESS_Recurring_Subscriber' ) ) {

		$subscriber   = new RPRESS_Recurring_Subscriber( $id_or_email, $by_user_id );
		...
		if ( $subscriber->id > 0 ) {
			...
			if ( ( $by_user_id && $id_or_email == $subscriber->user_id ) ) {
				...
				// If the email is in the RPRESS 2.6 Additional Emails
 ...
 * @return      string
 */
function rpress_stripe_get_customer_key() {

	$key = '_rpress_stripe_customer_id';
 ...
 * @return      bool
 */
function rpress_stripe_is_zero_decimal_currency() {
 ...
 */
function rpress_stripe_get_statement_descriptor() {
	$statement_descriptor = rpress_get_option( 'stripe_statement_descriptor', '' );
 ...
 */
function rpress_stripe_get_statement_descriptor_unsupported_characters() {
	 ...
	 */
	$unsupported_characters = apply_filters( 'rpress_stripe_get_statement_descriptor_unsupported_characters', $unsupported_characters  );
 ...
 * @return string $statement_descriptor Sanitized statement descriptor.
 */
function rpress_stripe_sanitize_statement_descriptor( $statement_descriptor ) {
	$unsupported_characters = rpress_stripe_get_statement_descriptor_unsupported_characters();
 ...
 * @return null|RPRESS_Stripe_Registry Null if the registry doesn't exist, otherwise the object instance.
 */
function rpress_stripe_get_registry( $name ) {
	switch( $name ) {
		case 'admin-notices':
			$registry = RPRESS_Stripe_Admin_Notices_Registry::instance();
			break;
		default:
			$registry = null;
			break;

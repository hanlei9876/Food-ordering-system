...
/**
 * Class RPRESS_Stripe_Rate_Limiting
 */
class RPRESS_Stripe_Rate_Limiting {
	...
	private function actions() {
		...
		add_action( 'rpress_before_purchase_form', array( $this, 'listen_for_recurring_card_errors' ), 0 );
	...
	private function filters() {
		...
		add_filter( 'rpress_checkout_button_purchase', array( $this, 'maybe_hide_purchase_button' ) );
	...
	public function setup_log_file() {
		...
		$this->filename   = wp_hash( home_url( '/' ) ) . '-rpress-stripe-rate-limiting.log';
	...
	public function has_hit_card_error_limit() {
		 ...
		 */
		$max_error_count = apply_filters( 'rpress_stripe_max_card_error_count', $max_error_count );
	...
	protected function update_rate_limiting_count( $blocking_id = '', $current_count = 0 ) {
		 ...
		 */
		$expiration_in_seconds = apply_filters( 'rpress_stripe_card_error_timeout', $expiration_in_seconds );
	...
	public function card_error_checks_enabled() {
		 ...
		 */
		$checks_enabled = apply_filters( 'rpress_stripe_card_error_checking_enabled', true );
	 ...
	 *
	 * ID is the IP address of the visitor. Prepends the value with `rpress_stripe_card_errors_` for use with the transient system.
	...
	public function listen_for_recurring_card_errors() {
		...
		// Get all of our RPRESS errors.
		$errors = rpress_get_errors();

		// If any of our errors are Stripe card errors from recurring, increment the card error counter.
		if ( isset( $errors['rpress_recurring_stripe_error'] ) && ! empty( $errors['rpress_recurring_stripe_error'] ) ) {

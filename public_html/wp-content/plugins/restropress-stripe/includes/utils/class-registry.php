 ...
 *
 * @todo Replace with RPRESS\Utils\Registry when RPRESS 3.x is required.
 *
 ...
 * @abstract
 */
abstract class RPRESS_Stripe_Utils_Registry extends ArrayObject {
	 ...
	 *
	 * @throws Exception if the item does not exist.
	 * @throws RPRESS_Stripe_Exceptions_Attribute_Not_Found if the attribute and/or item does not exist.
	 *
	...
	public function get_attribute( $key, $item_id ) {
		...
		if ( ! empty( $item[ $key ] ) ) {
			...
			throw RPRESS_Stripe_Utils_Exceptions_Attribute_Not_Found::from_attr( $key, $item_id );
		}

 ...
 */
const rpressStripeAdminNotices = function() {
	const notices = Array.prototype.slice.apply(
		document.querySelectorAll( '.rpress-stripe-admin-notice' )
	);
	...
	notices.forEach( function( noticeEl ) {
		...
		dismissButtonEl.addEventListener( 'click', function( e ) {
			...
			return wp.ajax.post(
				'rpress_stripe_admin_notices_dismiss_ajax',
				{
					id,
					nonce,
				}
...
// Wait for the DOM.
document.addEventListener( 'DOMContentLoaded', rpressStripeAdminNotices );

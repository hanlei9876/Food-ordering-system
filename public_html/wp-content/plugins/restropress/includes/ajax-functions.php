...
function rpress_ajax_add_to_cart() {
	if ( isset( $_POST['fooditem_id'] ) && isset( $_POST['fooditem_price'] ) ) {
		...
		if( apply_delivery_fee() ) :
			$return['delivery_fee'] = html_entity_decode( rpress_currency_filter( rpress_format_amount( get_delivery_fees() ) ), ENT_COMPAT, 'UTF-8' );
		endif;
		...
		$return['subtotal'] = html_entity_decode( rpress_currency_filter( rpress_format_amount( rpress_get_cart_subtotal() ) ), ENT_COMPAT, 'UTF-8' );

		$return['total'] = html_entity_decode( rpress_currency_filter( rpress_format_amount( rpress_get_cart_total() ) ), ENT_COMPAT, 'UTF-8' );
...
add_action( 'wp_ajax_rp_check_service_slot', 'rp_check_service_slot' );
add_action( 'wp_ajax_nopriv_rp_check_service_slot', 'rp_check_service_slot' );

function rp_check_service_slot() {
...
function rpress_show_products() {
	...
	$food_item_id = isset( $_POST['fooditem_id'] ) ? $_POST['fooditem_id'] : '';
	$price = isset( $_POST['fooditem_price'] ) ? $_POST['fooditem_price'] : '';
	...
	$fooditem_desc = get_post_field( 'post_content', $food_item_id );
	...
	if( !empty( $food_item_id ) ) {
		...
		$data = str_replace( '{Food_Description}', $food_title, $data );
	...
	$response = array(
		...
		'fooditem_description' 	=> apply_filters( 'rpress_fooditem_description' , $fooditem_desc ),
		'food_price'						=> $price,
	);
...
function rpress_ajax_update_cart_items() {
	if( isset( $_POST['fooditem_cartkey'] ) ) {
		...
		if( apply_delivery_fee() ) :
			...
			$return['delivery_fee'] = get_delivery_fees();
		endif;
...
function rpress_proceed_checkout() {
  ...
  if( $enable_minimum_order ) :
    ...
    $response = apply_filters( 'rpress_proceed_checkout_page', $response );
...
function rpress_check_new_orders() {
  ...
  if( is_array( $order ) && $order[0]->ID != $last_order ) {
  	...
  	$payment_gateway_label = rpress_get_gateway_checkout_label( rpress_get_payment_gateway( $payment_id ) );

  	$payment_gateway_label = sanitize_title( $payment_gateway_label );

  	$payment  = new RPRESS_Payment( $payment_id );

  	if ( $payment_gateway_label == 'cash-on-delivery' ) {
  		$notification = rpress_order_notification( $payment_id );
...
function rpress_order_notification( $payment_id ) {
  ...
  if ( !empty( $service_date ) ) {
  	$service_date = date_i18n( get_option( 'date_format' ), strtotime( $service_date ) );
  }
  ...
  $notification = array(
    ...
    'url'   => admin_url( 'edit.php?post_type=fooditem&page=rpress-payment-history&view=view-order-details&id=' . $payment_id )
  );

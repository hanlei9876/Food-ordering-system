 ...
 * @package     RPRESS
 * @subpackage  Gateways
 * @copyright   Copyright (c) MagniGenie
...
function rpress_paypal_process_pdt_on_return() {
	...
	if ( ! isset( $_GET['payment-id'] ) || ! isset( $_REQUEST['txn_id'] ) ) {
		return;
	}
	...
	if( ! empty( $_REQUEST['txn_id'] ) && $payment->ID > 0 ) {
		...
		$remote_post_vars = array(
			...
			'sslverify'   => false,
			'body'        => array(
				'tx'  => sanitize_text_field( $_REQUEST['txn_id'] ),
				'at'  => $token,
				'cmd' => '_notify-synch',
			)
		);

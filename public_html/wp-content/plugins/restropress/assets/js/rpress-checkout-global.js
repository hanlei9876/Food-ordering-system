window.RPRESS_Checkout = (function($) {
	...
	function init() {
		...
		$body.on('blur change', '.card-name', function() {
			...
			name_field.validateCreditCard(function(result) {
				if(result.card_type != null) {
					...
					$('#rpress-purchase-button').attr('disabled', 'disabled');
				} else {
					name_field.removeClass('error').addClass('valid');
					$('#rpress-purchase-button').removeAttr('disabled');
				}
			});
		});
		...
		$body.on('submit', '#rpress_payment_mode', function() {
			var gateway = $('#rpress-gateway option:selected').val();
			if( gateway == 0 ) {
		...
		$body.on('click', '#rpress_payment_mode_select input', function() {
			$('#rpress_payment_mode_select label.rpress-gateway-option-selected').removeClass( 'rpress-gateway-option-selected' );
			$('#rpress_payment_mode_select input:checked').parent().addClass( 'rpress-gateway-option-selected' );
		});
		...
		$checkout_form_wrap.on('keyup', '#rpress-discount', function (event) {
			if (event.keyCode == '13') {
				$checkout_form_wrap.find('.rpress-apply-discount').trigger('click');
			}
		});
		...
		$body.on('click', '.rpress_discount_link', function(e) {
			e.preventDefault();
			$('.rpress_discount_link').parent().hide();
			$('#rpress-discount-code-wrap').show().find('#rpress-discount').focus();
		});

		// Hide / show discount fields for browsers without javascript enabled
		$body.find('#rpress-discount-code-wrap').hide();
		$body.find('#rpress_show_discount').show();

		// Update the checkout when item quantities are updated
		$body.on('change', '.rpress-item-quantity', update_item_quantities);
	...
	function apply_discount(event) {
		...
		var $this = $(this),
			discount_code = $('#rpress-discount').val(),
		...
		var postData = {
			...
			code: discount_code,
			form: $( '#rpress_purchase_form' ).serialize()
		};

		$('#rpress-discount-error-wrap').html('').hide();
			...
			success: function (discount_response) {
				if( discount_response ) {
					if (discount_response.msg == 'valid') {
						$('.rpress_cart_discount').html(discount_response.html);
						...
						$('#rpress-discount', $checkout_form_wrap ).val('');
						...
						if( '0.00' == discount_response.total_plain ) {
							...
							$('#rpress_cc_fields,#rpress_cc_address,#rpress_payment_mode_select').slideUp();
							inputs.removeAttr('required');
							$('input[name="rpress-gateway"]').val( 'manual' );
							...
							$('#rpress_cc_fields,#rpress_cc_address').slideDown();
						...
						$('#rpress-discount-error-wrap').html( '<span class="rpress_error">' + discount_response.msg + '</span>' );
						$('#rpress-discount-error-wrap').show();
	...
	function remove_discount(event) {
		...
		$.ajax({
			...
			success: function (discount_response) {
				...
				$('.rpress_cart_amount').each(function() {
					if( rpress_global_vars.currency_sign + zero == $(this).text() || zero + rpress_global_vars.currency_sign == $(this).text() ) {
	...
	function update_item_quantities(event) {
		...
		var $this = $(this),
			quantity = $this.val(),
			key = $this.data('key'),
			fooditem_id = $this.closest('.rpress_cart_item').data('fooditem-id'),
			options = $this.parent().find('input[name="rpress-cart-fooditem-' + key + '-options"]').val();

		var rpress_cc_address = $('#rpress_cc_address');
		var billing_country = rpress_cc_address.find('#billing_country').val(),
			card_state      = rpress_cc_address.find('#card_state').val();

		var postData = {
			action: 'rpress_update_quantity',
			quantity: quantity,
		...
		$.ajax({
			...
			success: function (response) {
				...
				$('.rpress_cart_subtotal_amount').each(function() {
					$(this).text(response.subtotal);
				});

				$('.rpress_cart_tax_amount').each(function() {
					$(this).text(response.taxes);
				});

				$('.rpress_cart_amount').each(function() {
					$(this).text(response.total);
					$body.trigger('rpress_quantity_updated', [ response ]);
				});
			}
		}).fail(function (data) {
			if ( window.console && window.console.log ) {
...
window.jQuery(document).ready(RPRESS_Checkout.init);
...
function recalculate_taxes(state) {
	...
	if( ! state ) {
		state = $rpress_cc_address.find('#card_state').val();
	}

	var postData = {
		action: 'rpress_recalculate_taxes',
		billing_country: $rpress_cc_address.find('#billing_country').val(),
		state: state,
		card_zip: $rpress_cc_address.find('input[name=card_zip]').val()
	};
	...
	jQuery.ajax({
		...
		success: function (tax_response) {
			...
			if (current_ajax_count === ajax_tax_count) {
				jQuery('#rpress_checkout_cart_form').replaceWith(tax_response.html);
				jQuery('.rpress_cart_amount').html(tax_response.total);
